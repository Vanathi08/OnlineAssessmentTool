...
const Dashboard = () => {
  ...
  const checkAllCompletions = async (user) => {
    ...
    await Promise.all(
      topics.map(async (topic) => {
        try {
          const res = await axios.get(`https://onlineassessmenttool.onrender.com/api/module/completed/${user}/${topic.title.toLowerCase()}`);
  ...
  const handleCardClick = async (moduleName) => {
    try {
      const res = await axios.get(`https://onlineassessmenttool.onrender.com/api/module/check/${moduleName.toLowerCase()}`);
      if (res.data.exists) {
